...
DATABASE_URL = "sqlite:///./elearning_platform.db"
...
@app.put("/enrollments/{enrollment_id}/progress/", response_model=EnrollmentOut)
async def update_progress(enrollment_id: int, progress: int, db: Session = Depends(get_db)):
    db_enrollment = db.query(Enrollment).filter(Enrollment.id == enrollment_id).first()
    if not db_enrollment:
        raise HTTPException(status_code=404, detail="Enrollment not found")
    db_enrollment.progress = progress
    db_enrollment.completed = progress == 100
    db.commit()
    db.refresh(db_enrollment)
    return db_enrollment